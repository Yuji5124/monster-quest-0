import { DEV_MAJIN_CAVE_BALANCE, DEV_MAJIN_CAVE_HERO, MAJIN_CAVE_FLOOR_COUNT, MAJIN_CAVE_MONSTER_HOUSE_CANDIDATE_FLOORS } from "../config/majinCave.ts";
import type { MajinCaveHeroStats, MajinCavePhase, MajinCavePoint } from "../config/majinCave.ts";
...
export interface MajinCaveEnemyState {
  ...
  hp: number;
  defeated: boolean;
  /** ヒートで再生が止まっている残りphase数。0なら`regenPercentPerTurn`が毎phase働く。 */
  heatSuppressedTurnsRemaining: number;
}
...
export interface MajinCaveFloorState extends MajinCaveFloorLayout {
  ...
  /** 10F's Majin arrival is presentation-only and is shown once per generated FloorState. */
  majinRevealed: boolean;
...
export interface MajinCaveRunOptions {
  ...
  readonly now?: () => number;
  /** 主人公の現在レベルの能力値。省略時はDEV検証用の従来値(Lv9固定)。 */
  readonly hero?: MajinCaveHeroStats;
}
...
export class MajinCaveRunState {
  ...
  constructor(seed: number, options: MajinCaveRunOptions = {}) {
    ...
    this.hero = options.hero ?? DEV_MAJIN_CAVE_HERO;
    this.playerHp = this.hero.maxHp;
    this.playerMp = this.hero.maxMp;
  ...
  damageEnemy(enemy: MajinCaveEnemyState, amount: number, options: { readonly isHeat?: boolean } = {}): boolean {
    ...
    if (options.isHeat) {
      ...
      if (definition.heatSuppressionTurns) enemy.heatSuppressedTurnsRemaining = definition.heatSuppressionTurns;
    }
  ...
  applyEnemyRegen(): void {
    for (const enemy of this.getAliveEnemies()) {
      if (enemy.heatSuppressedTurnsRemaining > 0) { enemy.heatSuppressedTurnsRemaining -= 1; continue; }
      const definition = this.enemyDefinition(enemy);
      if (!definition.regenPercentPerTurn) continue;
      const healed = Math.round(definition.maxHp * definition.regenPercentPerTurn);
      enemy.hp = Math.min(definition.maxHp, enemy.hp + healed);
    }
  }

  /**
   * レベルアップ後の能力値へ差し替える。最大HP/MPの増加分だけ現在値も増やす
   * (全回復はしない)。レベルが下がることはない前提だが、上限は常に新しい最大値に合わせる。
   */
  applyHeroStats(hero: MajinCaveHeroStats): void {
    const hpGain = Math.max(0, hero.maxHp - this.hero.maxHp);
    const mpGain = Math.max(0, hero.maxMp - this.hero.maxMp);
    this.hero = hero;
    this.playerHp = Math.min(hero.maxHp, this.playerHp + hpGain);
    this.playerMp = Math.min(hero.maxMp, this.playerMp + mpGain);
  }

  /** MPが足りなければ何もせずfalseを返す。ヒートのMagic消費はここへ集約する。 */
  spendPlayerMp(amount: number): boolean {
    if (this.playerMp < amount) return false;
  ...
  recoverAtCurrentEntrance(): void {
    const recovered = this.hero.maxHp - this.playerHp;
    this.playerHp = this.hero.maxHp;
    this.playerMp = this.hero.maxMp;
  ...
  /** Returns true exactly once, when the 10F boss arrival presentation should play. */
  revealMajin(): boolean {
    const floor = this.currentFloor;
    if (floor.floorNumber !== MAJIN_CAVE_FLOOR_COUNT || !floor.bossPosition || floor.majinRevealed) return false;
    floor.majinRevealed = true;
    return true;
  }
  ...
  private recoverOnStair(): void {
    const recovered = Math.min(this.hero.maxHp - this.playerHp, DEV_MAJIN_CAVE_BALANCE.stairRecovery);
...
function createEnemies(runSeed: number, layout: MajinCaveFloorLayout, isMonsterHouse: boolean): MajinCaveEnemyState[] {
  ...
  for (let index = 0; index < Math.min(count, candidates.length); index += 1) {
    ...
    enemies.push({ id: `f${layout.floorNumber}-e${index + 1}`, definitionId: definition.id, position: candidates[index], hp: definition.maxHp, defeated: false, heatSuppressedTurnsRemaining: 0 });
  }
  if (layout.bossPosition) {
    const boss = MAJIN_CAVE_ENEMIES.majin;
    enemies.push({ id: "f10-majin", definitionId: boss.id, position: layout.bossPosition, hp: boss.maxHp, defeated: false, heatSuppressedTurnsRemaining: 0 });
  }
  return enemies;
}
