...
├─ maps/
│  ├─ <map_id>/
│  │  ├─ background.png  # 新規ローカルマップの正本
│  │  └─ collision.png   # 人間が修正する歩行Mask
│  ├─ tilesets/
│  │  ├─ tileset_base.png
│  │  └─ tileset_extra.png
│  └─ reference/
...
## ローカルマップ背景
- 新規の町・村・城・ダンジョンは高解像度の`background.png`を正本とする。詳細は `docs/MAP_SYSTEM.md`。
- `collision.png`は白=歩行可能、黒=歩行不可の編集可能なMaskを第一候補とする。背景画像の解析結果は必ず人間が修正可能にする。
- `tilesets/`内の既存タイル画像は削除しないが、新規マップ制作の標準ではない。
