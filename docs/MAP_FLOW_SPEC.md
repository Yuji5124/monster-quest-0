...
| 地域 | 区分 | 役割 / 備考 |
|---|---|---|
| はじまりのもり | 森 / 屋外探索 | 序盤用の独立ローカルマップ。高解像度背景＋Collision/Event/Object方式で制作し、ワールドマップの目的地ポイントにも追加する。プリン／たまゴースト等の序盤敵候補を配置する。正式No.は全体再整理時に決定 |
...
→ ワールドマップ（目的地ポイント選択）
→ No.02 はじまりのまち
→ 追加採用「はじまりのもり」を含む序盤の屋外探索
→ No.03 ビーエのむら
→ 以後、拠点→屋外探索→拠点→ダンジョンのリズムを基本に進行
→ No.04〜05 レインランド方面
→ No.06 ザボンのむら
→ No.07〜14 中盤地域
→ No.15 ふっかつのほこら
...
既存No.01〜20の番号順は既存データ互換の管理基準とする。**物語上の実移動順と最終再採番は別管理**とし、中盤の寄り道・再訪・追加森マップ・分岐を許容する。
