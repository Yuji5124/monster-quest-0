...
主要更新:
- 「はじまりのもり」→「ビーエのもり」。
- レインランドのむらは最新メイン順から外す。
- いしのむら→終盤18「いしのまち」。
- 09でタロサ一時参加、攻略後から同行。
- 10でミレイ初登場、11で古代文字の知識を理由に参加。
- 13 ゆうしゃのたて、14 ゆうしゃのけん、15 ゆうしゃのかんむり。
- 14 ポサロ城にバクラー。
- 16 デーマス。
- 19 バトラスは毒 / タロサの毒の矢。
- 20 オロチへの道は縦シューティング。
- 完全な物語正本は `STORY_FLOW.md`、順番正本は `PLAY_ORDER_SPEC.md`。
...
## 追加マップ候補整理（2026-09-22）

最新の物語順1〜20は維持する。
追加マップ候補は `MAP_CANDIDATES.md` で管理する。
...
特殊区間の配置確定:
- **06 レインランドじょう内部 = ブロック／ボクセル3Dを正式採用**
- **20 オロチへの道 = 縦シューティングを正式採用**
- 横アクション専用区間 = 配置TBD
- 縦アクション専用区間 = 配置・仕様TBD

C02/C03はユーザーの明示指示なしに復活させない。
...
## 3. 最新キャラクター結論（2026-09-22）

### 主人公
- 男性。
- 別のモンスタークエスト作品／別バージョン世界の元NPC。
- 元世界では自由に行き先を決められず、「勇者になりたい／自由に歩きたい」という願いを持っていた。
- 世界混線によりMQ0で一度だけ主人公になる。
- 生まれつきの勇者・特別な血筋ではない。
- 真の最終解決後は「ここに のこる」／「もとの せかいへ かえる」の選択式。
- 残留ルートではミレイとレインランドへ戻り、後に娘レイを育てる。ミレイはレイ出産時に亡くなる。
- 帰還ルートではわたべと元世界へ帰り、本来のNPCへ戻るが、MQ0の記憶は残る。

### タロサ
- 弓・武器アクションを主軸とする。
- 努力を信じる純粋な村のヒーロー。
- MQ0本編中にはデスタロッサを登場させない。
- 努力では越えられない力の差を知り、MQ0後に悪魔の「限界をなくす力」の誘いへ向かい、やがてデスタロッサへ至る未来を持つ。
- バトラス戦では毒付与を含むタロサの弓／武器アクションが重要。

### ミレイ
- レインランドの姫。
- 城で待つ役割だけではなく、自分の足で世界を見たいという願いを持つ。
- 世界混線をきっかけに本来の役割から外れ、魔法へ目覚める。
- 主人公に恋心を抱く。
- 未来の断片として「姫を産む」未来を感じる。
- 残留ルートでは主人公との娘レイを出産し、その際に亡くなる。

### わたべ
- 主要3人とは別の特殊参加。
- 現実世界側／ゲーム世界の外側に近い存在。
- 「もういちど」以降に参加し、固有攻撃「とくだいヒット」を使う。
- オロチゾンビ再戦の真の解決に必要。
- 最終解決後は現実世界側へ帰還する。

旧「主人公は必ずわたべと元世界へ帰る」固定エンディングはSUPERSEDED。
...
物語順は `PLAY_ORDER_SPEC.md` を正本とする。旧No.01〜20は実装互換用であり、物語順には使用しない。
...
| 04 | ビーエのむら | バグ景観 |
| 05 | レインランドのもり | 王都への探索 |
| 06 | レインランドじょうかまち / レインランドじょう | **城内部ブロック／ボクセル3D正式採用** |
| 07 | まじんのどうくつ | 主人公1人 / ターン制ダンジョンRPG |
| 08 | ザボンのむら | タロサ出身地 |
| 09 | いわやまのどうくつ | タロサ一時参加 / 縦シューティング要素 |
| 10 | かくれざと | ミレイと出会う |
| 11 | みずうみの古城 | ミレイ参加 |
| 12 | 港町ダコハ | 情報拠点 |
| 13 | コタンカイムの洞窟 | ゆうしゃのたて |
| 14 | ポサロ城 | バクラー / ゆうしゃのけん |
| 15 | ふっかつのほこら | ゆうしゃのかんむり |
| 16 | デーマスのとう | デーマス |
| 17 | ぬまちのどうくつ | 多数敵アクション |
| 18 | いしのまち | 終盤準備 |
| 19 | バトラスのとりで | 毒攻略 |
| 20-A | オロチへの道 | **縦シューティング正式採用** |
| 20-B | オロチのしろ | オロチまおう |
| 20-C | 最終地点 | オロチゾンビ / もういちど |
...
旧コード・アセット上の互換No.一覧は `MAP_FLOW_SPEC.md` で管理する。
...
## 10. 真のエンディング
- 主人公の結論は二択。
- 「ここに のこる」: ミレイとレインランドへ戻り、後に娘レイを育てる。ミレイは出産時に亡くなる。
- 「もとの せかいへ かえる」: わたべと元世界へ帰還し、本来のNPCへ戻る。冒険の記憶は残る。
- タロサは強さを求めて旅立ち、その後のデスタロッサへつながる伏線を残す。
- ミレイの未来・レイ・死は本編で長文説明せず、未来描写と余韻で示す。
- わたべは最終解決後、現実世界側へ戻る。
