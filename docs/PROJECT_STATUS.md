...
## No.05 レインランドじょう（通常RPG方式の城内）= PARTIAL（2026-09-19着手・2026-09-20導線確定）
...
**背景は仮**: 城内の歩行用の画像が未提供のため、`map.json`の`assetStatus: "DEV_PLACEHOLDER"`の単色レイアウト画像を使っている。`tools/build_rainland_castle_placeholder.py`が`background.png`と`collision.png`を1つの定義から生成する。**正式画像への差し替えはデータだけ**（画像2枚＋`map.json`。Sceneのコードは変更しない）。既存の`レインランドじょう_イメージ.png`（城の外観、町の入場演出用）と`レインランドじょう_マイクラ風.png`（一人称のブロック城、将来のVoxel化用の参照）は2D歩行背景に使っていない。
