...
- 歩行キャラクターとUIは、FC〜初期SFCを感じるレトロゲーム表現を基本とする。ローカルマップ背景は高解像度の1枚絵を使用できる。
...
## マップ背景
- 新規ローカルマップは `MAP_SYSTEM.md` に従い、ChatGPT等で生成した高解像度の`background.png`を正本にできる。
- 歩行可能領域を判定しやすい見下ろし寄りのRPG視点、明確な道・入口・障害物輪郭、過度でない影、少ない巨大前景を優先する。
- Collisionを作りやすい構図を、美観と同じ優先度で評価する。
- `assets/maps/tilesets/` とTiled用タイル画像は既存実装のlegacy / referenceとして保持する。新規背景をタイルベースへ固定しない。
