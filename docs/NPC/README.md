...
このフォルダは、町・村ごとのNPC会話を再編集する作業資料を置く場所。`NPC_DIALOGUE_MASTER.md` が提供された場合は、本文についてそちらを正本とする。現在は未提供のため、ここにある `SOURCE_DRAFT_EXISTS` / `REDUCING` 文書を正式会話本文として実装しない。
...
- 町・村・城内部とNPC人数は、旧案を機械的に半減せず、地域の役割・話題の重複・目標プレイ時間に合わせてコンパクトに再設計する。
...
地域ファイルに本文を移す際は、最新の話題バランス・地域ごとの人数目安を先に確認する。
