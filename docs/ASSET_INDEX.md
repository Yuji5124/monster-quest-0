...
| `assets/characters/playable/watabe_walk.png` | わたべ 歩行スプライト | READY_TO_IMPORT |
...
## 5. 戦闘背景
現在の方針は「FC風の単純背景」より一段きれいな、ゲーム画面で映えるレトロRPG背景。
...
公開用ではネタバレカードの表示名を `？？？` にしてよいが、内部ファイル名・IDは実装上の正式名を維持する。
